...
const ChooseExchange = ({ data: currentData, addData }) => {
  ...
  // Bezwzględnie dodajemy Zondę do listy, nawet jeśli nie jest znaleziona w accountsConfig
  if (!exchanges.filled.some(e => e.code === 'zonda') && 
      !exchanges.unfilled.some(e => e.code === 'zonda')) {
    ...
    if (zondaConfig) {
      ...
      // Tworzę config Zondy jeśli nie istnieje
      exchanges.unfilled.push({
        code: 'zonda',
        display: 'Zonda',
        class: 'exchange',
        cryptos: [coin]
