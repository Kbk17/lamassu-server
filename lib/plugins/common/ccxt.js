...
const zonda = require('../exchange/zonda')
...
function buildMarket (fiatCode, cryptoCode, serviceName) {
  console.log(`Budowanie rynku dla ${cryptoCode}/${fiatCode} na giełdzie ${serviceName}`);
  ...
  // Zonda używa formatu z myślnikiem zamiast ukośnika
  if (serviceName === 'zonda') {
    const market = `${cryptoCode}-${fiatCode}`;
    console.log(`Specjalny format rynku dla Zondy: ${market}`);
    return market;
  }
