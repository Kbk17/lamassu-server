...
function trade (side, account, tradeEntry, exchangeName) {
  ...
  try {
    ...
    let ccxtExchangeName = exchangeName;
    
    if (EXCHANGE_NAME_MAP[exchangeName]) {
      // Jeśli mamy mapowanie dla tej giełdy, spróbuj użyć pierwszej dostępnej nazwy
      for (const nameOption of EXCHANGE_NAME_MAP[exchangeName]) {
        if (typeof ccxt[nameOption] === 'function') {
          console.log(`Użycie alternatywnej nazwy dla ${exchangeName}: ${nameOption}`);
          ccxtExchangeName = nameOption;
          break;
        }
      }
    }
    
    console.log(`Inicjalizacja exchange ${exchangeName} (CCXT: ${ccxtExchangeName})`);
    const exchange = new ccxt[ccxtExchangeName](loadConfig(account))
    ...
    console.error(`Błąd podczas handlu na ${exchangeName}:`, e);
